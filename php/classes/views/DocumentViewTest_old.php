...
//        $this->showSelectBlock("dst", $headerParams["dstName"], $ar, "width: 30%");
    ...
    private function showOld(){
        ...
        $executed = ($doc->isExecuted()) ? "<span style='color:green;font-size:15px;'> (проведен)</span>" : "<span style='color:red;font-size:15px;'> (не проведен)</span>";
        $headerParams = $this->typeParams[$this->type];
        $data = [
            "title" => $this->typeParams[$this->type]["name"]. $executed
        ];
        if ($this->print){
        ...
        if (!$this->print){
            ...
            else if (($this->type == "transfer") && (!$doc->isExecuted())){
                
                $profile = $_COOKIE["login"];
                $dst = new \Subcount($this->header["dst"]);
                $src = new \Subcount($this->header["src"]);
                $giver = $src->getParams("inCharge");
                $taker = $dst->getParams("inCharge");
                if ($profile == $giver){
                    $buttonList[] = "delete";
                }
                if ($profile == $taker){
